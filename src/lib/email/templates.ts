...
export function abandonedCartEmail(input: {
  ...
  const content =
    ...
    `<p style="margin:0;font-size:13px;color:${C.muted};">Voor 09:00 besteld, vanavond in huis. Met de gratis KLUSRPAS pak je bovendien altijd extra voordeel.</p>`;
  return {
    subject: "Je winkelwagen staat nog klaar bij KLUSR",
    html: layout({
      title: "Je winkelwagen wacht nog op je",
      preheader: "Rond je bestelling af — voor 09:00 besteld, vanavond in huis.",
      content,
...
export function orderConfirmationEmail(order: Order): { subject: string; html: string; text: string } {
  ...
  const addressBlock =
    ...
    `Voor 09:00 besteld, vanavond in huis<br>` +
